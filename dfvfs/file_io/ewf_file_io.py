...
class EWFFile(file_object_io.FileObjectIO):
  ...
  def _OpenFileObject(self, path_spec):
    ...
    if parent_path_spec.IsSystemLevel():
      # Have pyewf deal with the maximum number of open files.
      filenames = pyewf.glob(parent_path_spec.location)
      ewf_handle.open(filenames)

    else:
      # Note that we cannot use pyewf's glob function here since it does not
